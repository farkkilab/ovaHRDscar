...
calc.ai_new<-function(seg, chrominfo, min.size=1e6, cont = 0,ploidyByChromosome=TRUE, shrink=TRUE){
  ...
  AI <- rep(NA, nrow(seg))
  seg <- cbind(seg, AI) #Here it will be stored the kind of AI,  #0 = no AI, 1=telomeric AI, 2=interstitial AI, 3= whole chromosome AI
  ...
  for(j in samples){
    ...
    for(i in unique(sample.seg[,2])){
      ...
      if(ploidyByChromosome){
        ...
        #Next lines will add info about AI, and add a 2 if there is an allelic imbalance sample.seg[,7] == sample.seg[,8], also add ploidy to all sample.seg
        if(ploidy %in% c(1,seq(2, 200,by=2))){
      ...
      #Then check if the first AI event and last AI event are before and after the centromere. If yes then change these AI column to 1
      ...
      #If there is just only one AI event in the chromosome, then add value 3= whole chromosome AI
  ...
  #After getting the kind of AI, then count for each sample the number of each AI
  samples <- as.character(unique(seg[,1]))
