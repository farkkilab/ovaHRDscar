...
#Segsize is the minimun size of two Allelic Imbalances, mindistance is the distance between them, tandem elements is the number of segments inside a large scale transition
LSTs <- function(seg, chrominfo = chrominfo_grch38, segsizes=10e6, mindistance=3e6, tandemelements=2, getpositions = FALSE){
  ...
  for(j in samples){
    ...
    for(i in unique(sample.seg[,2])){
      ...
      #Removing  segments less than mindistance for the p.arm. Then smoothing the resultan segmnents according to their CNvalues and mindistance
      ...
      #Removing segments less than mindistance for the q.arm. Then smoothing of the resultan values acording to their CNvalues and mindistance
